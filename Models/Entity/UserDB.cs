...
namespace Models.Entity
{
    public class UserDB : DB
    {
        ...
        public int FindUser(string LoginUser, string PassUser)
        {
            ...
            MySqlCommand commandChooseUser = new MySqlCommand("SELECT * FROM `users` WHERE `login` = @uL AND `password` = @uP", db.GetConnection());
        ...
        {
            MySqlCommand commandChooseUser = new MySqlCommand("INSERT INTO `users`(`login`, `password`) VALUES (@uL, @uP)", db.GetConnection());
            commandChooseUser.Parameters.Add("@uL", MySqlDbType.VarChar).Value = LoginUser;
            commandChooseUser.Parameters.Add("@uP", MySqlDbType.VarChar).Value = PassUser;
